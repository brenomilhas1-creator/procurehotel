...
export interface ProductPriceOption {
  product_id: string;
  product_name: string;
  options: (SupplierPrice & { supplier_name: string })[];
  best: (SupplierPrice & { supplier_name: string }) | null;
}
...
/**
 * M11: Preços agrupados por produto com melhor opção e todas as alternativas.
 * Usado pelo Pedido Rápido para mostrar onde está mais barato e dar opção de mudar.
 */
export async function getProductPricesWithAlternatives(productIds: string[]): Promise<ProductPriceOption[]> {
  const c = sb();
  if (!c || productIds.length === 0) return [];
  const { data, error } = await c.from('supplier_prices')
    .select('id, supplier_id, product_id, price, unit_price, package_qty, min_order_qty, currency, is_current, source, valid_from, suppliers!inner(name)')
    .in('product_id', productIds)
    .eq('is_current', true)
  ...
  for (const p of (products || []) as any[]) productNames.set(p.id, p.master_name);

  const byProduct = new Map<string, ProductPriceOption>();
  for (const id of productIds) {
    byProduct.set(id, { product_id: id, product_name: productNames.get(id) || '?', options: [], best: null });
  }
  for (const p of (data || []) as any[]) {
    const opt = { ...p, supplier_name: p.suppliers?.name || 'Desconhecido' };
    const group = byProduct.get(p.product_id);
    if (group) {
      group.options.push(opt);
      if (!group.best || opt.unit_price < group.best.unit_price) {
        group.best = opt;
      }
    }
  }
...
export async function getPriceHistory(productId: string, days = 90): Promise<SupplierPrice[]> {
  ...
  const { data } = await c.from('supplier_prices')
    .select('id, supplier_id, product_id, price, unit_price, package_qty, min_order_qty, currency, is_current, source, valid_from, created_at')
...
export async function getPriceTrends(productIds: string[]): Promise<Record<string, PriceTrend>> {
  const c = sb();
  if (!c || productIds.length === 0) return {};
  // Buscar histórico da tabela supplier_price_history (até 5 mais recentes por produto)
  const { data, error } = await c.from('supplier_price_history')
  ...
  // Agrupar por product_id (preço mais recente primeiro)
  const byProduct = new Map<string, any[]>();
  for (const id of productIds) byProduct.set(id, []);
  for (const row of (data || []) as any[]) {
    if (byProduct.has(row.product_id)) byProduct.get(row.product_id)!.push(row);
  }
  ...
  for (const [pid, rows] of byProduct) {
    if (rows.length === 0) {
      trends[pid] = {
        product_id: pid, current_price: null, previous_price: null,
        delta_eur: null, delta_pct: null, trend: 'new',
        last_update: null, history_count: 0,
      };
      continue;
    }
    const current = rows[0]; // mais recente
    const previous = rows.length > 1 ? rows[1] : null;
    const currPrice = Number(current.unit_price);
    const prevPrice = previous ? Number(previous.unit_price) : null;
    let trend: 'up' | 'down' | 'flat' | 'new' = 'new';
    let delta_eur: number | null = null;
    let delta_pct: number | null = null;
    if (prevPrice !== null && prevPrice > 0) {
      delta_eur = currPrice - prevPrice;
      delta_pct = ((currPrice - prevPrice) / prevPrice) * 100;
      // Só marcar como up/down se diferença > 0.5%
      if (Math.abs(delta_pct) < 0.5) trend = 'flat';
      else if (delta_pct > 0) trend = 'up';
      else trend = 'down';
    }
    trends[pid] = {
      product_id: pid,
      ...
      delta_pct,
      trend,
      last_update: current.recorded_at,
...
export async function getOrderItems(orderId: string): Promise<any[]> {
  ...
  const { data } = await c.from('purchase_order_items')
    .select('id, product_id, quantity, unit_price, line_total, raw_line, products(master_name, brand, unit)')
    .eq('order_id', orderId);
  return (data || []) as any[];
}
...
export interface PendingQuote {
  ...
  status: 'pending' | 'quoted' | 'ordered' | 'rejected' | 'cancelled';
  ...
  updated_at: string;
  // joins
  suppliers?: { name: string; contact_email: string | null; contact_phone: string | null } | null;
  products?: { master_name: string; category: string | null; unit: string } | null;
}

export async function listPendingQuotes(opts: { status?: string; supplierId?: string; limit?: number } = {}): Promise<PendingQuote[]> {
  const c = sb();
  if (!c) return [];
  let q = c.from('pending_quotes')
    .select('id, user_id, supplier_id, product_id, raw_line, requested_quantity, unit_of_measure, status, quoted_price, quoted_at, quote_reference, quote_notes, source_order_id, notes, created_at, updated_at, suppliers!pending_quotes_supplier_id_fkey(name, contact_email, contact_phone), products!pending_quotes_product_id_fkey(master_name, category, unit)')
    .order('created_at', { ascending: false })
...
export interface PendingQuoteInput {
  raw_line: string;
  supplier_id?: string | null;
  product_id?: string | null;
  requested_quantity?: number | null;
...
export async function createPendingQuote(input: PendingQuoteInput): Promise<{ ok: boolean; error?: string; id?: string }> {
  const c = sb();
  if (!c) return { ok: false, error: 'no_client' };
  const { data, error } = await c.from('pending_quotes').insert({
...
export async function updatePendingQuote(id: string, updates: Partial<PendingQuote>): Promise<{ ok: boolean; error?: string }> {
  const c = sb();
  if (!c) return { ok: false, error: 'no_client' };
  const { error } = await c.from('pending_quotes').update(updates).eq('id', id);
  if (error) return { ok: false, error: error.message };
  return { ok: true };
}

/**
 * Converte um pending quote em supplier_price + purchase_order_item.
 * Chamada tipicamente após o fornecedor responder com o preço.
 */
export async function quotePendingToOrder(opts: {
  pendingId: string;
  quotedPrice: number;
  supplierId: string;          // obrigatório no momento da cotação
  productId: string;           // obrigatório no momento da cotação
  orderId?: string;            // se null, cria uma nova PO
  companyId?: string | null;
}): Promise<{ ok: boolean; error?: string; order_id?: string; price_id?: string }> {
  const c = sb();
  if (!c) return { ok: false, error: 'no_client' };
  try {
    // 1. Criar/atualizar supplier_price
    const { data: priceRow, error: priceErr } = await c.from('supplier_prices')
      .upsert({
        supplier_id: opts.supplierId,
        product_id: opts.productId,
        price: opts.quotedPrice,
        unit_price: opts.quotedPrice,
        package_qty: 1,
        min_order_qty: 1,
        is_current: true,
        source: 'manual',
        notes: 'Cotação via pendentes',
      }, { onConflict: 'product_id,supplier_id' })
      .select('id')
      .single();
    if (priceErr) return { ok: false, error: 'price:' + priceErr.message };

    // 2. Atualizar pending_quote para 'quoted'
    await c.from('pending_quotes').update({
      status: 'quoted',
      quoted_price: opts.quotedPrice,
      quoted_at: new Date().toISOString(),
      supplier_id: opts.supplierId,
      product_id: opts.productId,
      source_order_id: opts.orderId || null,
    }).eq('id', opts.pendingId);

    return { ok: true, price_id: priceRow?.id };
  } catch (e: any) {
    return { ok: false, error: e?.message || String(e) };
  }
}

export async function deletePendingQuote(id: string): Promise<{ ok: boolean; error?: string }> {
  const c = sb();
  if (!c) return { ok: false, error: 'no_client' };
  const { error } = await c.from('pending_quotes').delete().eq('id', id);
  if (error) return { ok: false, error: error.message };
  return { ok: true };
}

export async function getPendingQuotesBySupplier(): Promise<Array<{ supplier_id: string | null; supplier_name: string; supplier_email: string | null; supplier_phone: string | null; pending_count: number; total_count: number }>> {
  const c = sb();
  if (!c) return [];
  const { data, error } = await c.from('pending_quotes_by_supplier').select('*');
  if (error) return [];
  return (data || []) as any;
}
