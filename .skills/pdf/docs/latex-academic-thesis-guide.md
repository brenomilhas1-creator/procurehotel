...
> **Cloud sandbox note**: this overlay ships in
> `packages/cloud-runtime/skill-overlays/pdf/docs/` (the cloud-runtime build).
> The cloud image installs `texlive-xetex` + `texlive-latex-recommended`
> + `texlive-lang-chinese` + `texlive-fonts-recommended` via apt, so the
> **active** compile chain is `xelatex` + `bibtex` (or `latexmk -xelatex`).
> `tectonic` is still NOT installed in the cloud image (no reachable GitHub
> release mirror from the buildkit cluster — see
> `packages/cloud-compute-runtime/Dockerfile` §Tectonic for the rationale).
> Desktop builds using the same skill may have `tectonic` available;
> recipes below that show both options are correct in either environment.
