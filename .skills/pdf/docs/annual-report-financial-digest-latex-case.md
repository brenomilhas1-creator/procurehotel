...
### 3.2 Image reading per page for complex financial tables / charts

For complex annual-report financial tables, render each target page to a PNG with
`read_pdf_vision.py` and have the model read it itself. Do not pass ranges
for these target pages — one tool call per page keeps per-page fidelity intact.
...
python3 -m scripts.read_pdf_vision \
  ...
  --pages 7 \
  --json
# stdout is a render manifest. Read `pages[0].path` (a single PNG) with
# the native `read` tool first; if the manifest flags it `oversized: true`,
# call `matrix.image_understanding` MCP with the same path. Use the prompt
# below in either tool call.
```

Prompts used in this run (the model now passes the prompt directly to the
`read` / `image_understanding` tool call, instead of `--prompt` on the
script):

```text
请逐字提取本页‘主要财务数据/会计数据和财务指标摘要’表格，保留所有行名、年份和数值，输出 Markdown 表格。
```
...
Recovery: `image_understanding` may return `gemini analysis failed: unexpected
end of JSON input` (transient upstream LLM hiccup). Retry the same tool call
once; if it repeats and the text layer is clean, use pdfplumber output for
that page and cross-check against adjacent segment tables.
...
| Vision transient failure | `image_understanding` returns `unexpected end of JSON input` | Retry the same `image_understanding` call once; fallback to pdfplumber if text layer is clean. |
...
1. Preflight: verify `pdfinfo`, `pdftotext`, `pdfplumber`, `pypdf`, `xelatex`/`latexmk`.
2. Locate first: page count, outline/TOC, identify financial-summary and segment pages.
3. Extract:
   - pdfplumber for clean text pages;
   - image reading one page at a time for complex tables — `read_pdf_vision.py`
     renders the PNG, the model reads it (native `read` first,
     `matrix.image_understanding` MCP for `oversized: true` pages).
