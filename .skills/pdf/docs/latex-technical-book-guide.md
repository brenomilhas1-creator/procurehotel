...
> **active** compile chain is `latexmk -xelatex` (or
> `xelatex` + `bibtex` for the manual two-pass flow). The template's
> README and the `latexmk -xelatex -interaction=nonstopmode main.tex`
> commands below are exactly what the cloud sandbox runs.
