...
- The source is scanned/image-only content with no text layer; use the
  image-reading route (`read_pdf_vision.py` per page → native `read` /
  `matrix.image_understanding` MCP) to recover the text before
  translation.
