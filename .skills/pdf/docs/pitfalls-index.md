...
>
> - `<SKILL>` → `/Users/<user>/.mavis*/.builtin-skills/pdf`
> - `<SKILL_XLSX>` → `/Users/<user>/.mavis*/.builtin-skills/xlsx`
...
## P4 — Multi-jurisdiction research with mavis-team
...
STEP 1 — Plan structure (timeout-with-degrade is mandatory)
  Build a mavis-team plan with N parallel research workers + 1 synthesis
...
STEP 4 — Launch + monitor
  mavis team plan run <TMP>/plan.yaml
...
STEP 3 — Once target pages identified
  ...
  - Chart / diagram / financial table → image reading PER PAGE
    (mandatory, see §P7). The script renders one PNG per page; the model
    reads each PNG itself with native `read` (or `matrix.image_understanding`
    MCP for any `oversized: true` page).
...
STEP 2 — Image reading per page (MANDATORY — pdfplumber FORBIDDEN here)
  ...
  # never have.
  #
  # The 2026-06 refactor of read_pdf_vision.py no longer calls any vision
  # MCP from inside the script. The script just renders each requested
  # page to its own PNG; the model then reads each PNG itself — first
  # try the native `read` tool, fall back to `matrix.image_understanding`
  # MCP for any page flagged `oversized: true` in the manifest. One tool
  # call per page. Do not batch pages together.

  for p in $PAGES; do
    python3 -m scripts.read_pdf_vision \
      --input "{PDF_PATH}" --pages $p \
      --json 2><TMP>/vision-p$p.log > <TMP>/vision-p$p.json
    # <TMP>/vision-p$p.json now contains a render manifest with
    # `pages[].path` for the one requested page. The model then issues
    # one `read` (or `image_understanding`) call per path and asks for
    # the same TSV schema the old `--prompt` did:
...
STEP 3 — Parse vision output
  # The model now writes the TSV directly to <TMP>/vision-p$p.tsv as it
  # reads each page (no `chunks[0].text` indirection). Load each file and
  # normalise into the row dict below. If the model returns prose instead
  # of TSV (e.g. a chart legend it couldn't put in cells), catch it in
  # the page-level review and re-issue the read with a sharper prompt.
  python3 <<EOF
  import pathlib
  rows = []
  for p in PAGES:
      tsv = pathlib.Path(f"<TMP>/vision-p{p}.tsv").read_text(encoding="utf-8")
...
CONSTRAINTS:
  ...
  - Image reading MUST be per-page (`--pages N`). A range like
    `--pages 1-30` renders 30 PNGs to disk and the model would have to
    call `read` on each one — slow and easy to mis-attribute values. The
    previous chunked-stitched vision flow had exactly this failure mode
    and was removed in 2026-06.
  - Don't 2>/dev/null the script call — the script's stderr is the only
    signal for pdfinfo failures, missing PNGs, or oversized page flags.
```
