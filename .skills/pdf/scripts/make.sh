...
#   packages/daemon/skills/pdf/scripts/make.sh
# Patches cmd_check to probe tectonic (LATEX routes), and short-circuits
# cmd_fix to check-only when CLOUD_SANDBOX=1 / /.dockerenv (deps baked).
...
cmd_check() {
  ...
  # CLOUD-OVERLAY: xelatex / latexmk probe for LATEX_THESIS / LATEX_TECHNICAL_BOOK routes.
  # The image ships TeX Live (`texlive-xetex` + `texlive-latex-recommended`); the templates
  # under `templates/latex-academic-thesis/` and `templates/latex-technical-book/` compile
  # with `xelatex` / `latexmk -xelatex`. `tectonic` itself is still NOT installed (no
  # reachable GitHub release mirror from the buildkit cluster — see Dockerfile §Tectonic
  # for the rationale and the path-(a) future). Probe xelatex / latexmk as the active
  # chain and emit tectonic as an optional WARN so the check stays green when both are
  # absent.
  if command -v xelatex >/dev/null; then
    green "  OK xelatex $(xelatex --version 2>/dev/null | head -1 | awk '{print $2}') (LATEX_THESIS / LATEX_TECHNICAL_BOOK enabled via TeX Live)"
  else
    red "  FAIL xelatex not found — LATEX_THESIS / LATEX_TECHNICAL_BOOK routes will fail (apt install texlive-xetex)"
    ok=false
  fi
  if command -v latexmk >/dev/null; then
    green "  OK latexmk $(latexmk --version 2>/dev/null | head -1)"
  else
    yellow '  WARN latexmk not found — `latexmk -xelatex main.tex` workflow will fall back to manual `xelatex && bibtex && xelatex`'
  fi
  if command -v tectonic >/dev/null; then
    green "  OK tectonic $(tectonic --version 2>/dev/null | head -1 | awk '{print $2}') (preferred upstream, single-binary — used if available)"
  else
    yellow "  WARN tectonic not found — path (a) of Dockerfile §Tectonic is not active. xelatex / latexmk from TeX Live is the active chain."
  fi
...
cmd_fix() {
  ...
  # In cloud sandbox all deps are baked into the image; running pip /
  # npm install over a read-only-ish base layer wastes time and may fail
  # on missing root. Short-circuit to check-only.
  if [ "${CLOUD_SANDBOX:-0}" = "1" ] || [ -f /.dockerenv ]; then
    bold "cloud sandbox: deps baked into image, running check-only"
    cmd_check
    return $?
  fi
  # ────────────────────────────────────────────────────────────────────────
